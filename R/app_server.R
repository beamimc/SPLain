server <- function(input, output, session, data) {
  ...
  output$current_comparison <- renderText({
    ...
    paste("Current: ", conds$cd2, "/", conds$cd1)
  })
  output$current_fdr <- renderText({
    fdr <- applied_fdr()
    paste("Current FDR: ", fdr)
  })
  ...
  isoformAnalysisServer("isoform", se, exons, filtered_dtu_df, 
                        sig_res, selected_conditions)
